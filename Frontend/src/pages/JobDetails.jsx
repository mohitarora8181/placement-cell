...
const JobDetails = () => {
  ...
  useEffect(() => {
    axios.get(`https://placement-cell-iczn.onrender.com/api/jobs/${jobId}`)
      .then(response => {
        setJob(response.data.job);
        if (response.data.job.shortlistedStudents && typeof response.data.job.shortlistedStudents === 'string') {
    ...
    if (typeof shortlistedStudents === 'string') {
      return (
        <Typography variant="body1" color="primary">
          <a href={shortlistedStudents} target="_blank" rel="noopener noreferrer">
        ...
        <div className='mx-auto w-[80%]'>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px' }}>
            <Typography variant="h5" gutterBottom style={{ fontWeight: 'bold' }}>
              {view === 'applied' ? 'Students who applied' : 'Shortlisted Students'}:
            </Typography>
            <div>
              <Button variant="outlined" color="primary" onClick={() => setView('applied')} style={{ marginRight: '10px' }}>
                View Applied Students
              </Button>
              <Button variant="outlined" color="primary" onClick={() => setView('shortlisted')}>
                View Shortlisted Students
              </Button>
            </div>
          </div>
          ...
          {view === 'applied' ? renderStudentCards(job.applicants || []) : renderShortlistedStudents()}
        </div>
      </div>
    </>
  ) : (
